...
const ProductCard = ({products}) => {
    const {currency,addToCart,removeItemFromCart,cartItems,navigate} = useAppContext()
    
    return products && (
        <div onClick={()=>{navigate(`/products/${products.category.toLowerCase()}/${products._id}`);scrollTo(0,0)}} 
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-end justify-between mt-3">
                    <p className="md:text-xl text-base font-medium text-indigo-500">
                        {currency}${products.offerPrice} {" "} <span className="text-gray-500/60 md:text-sm text-xs line-through">{currency}${products.price}</span>
                    </p>
                    <div onClick={(e)=>{e.stopPropagation();}} className="text-indigo-500">
                        {!cartItems[products._id] ? (
                            ...
                            md:w-[80px] w-[64px] h-[34px] rounded text-indigo-600 font-medium cursor-pointer" 
